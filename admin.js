...
const supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, { auth: { persistSession: true,
...
function esc(value){return String(value ?? '').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>
    ...
    supabaseClient.from('gaming_reports').select('id,report_date,created_at,updated_at,follow_up_report,created_by').order('report_date',{ascending:false}).order('created_at',{ascending:false});
    if(error)throw error;
    const {data:signoffs,error:signoffError}=await
    supabaseClient.from('report_signoffs').select('report_id,admin_name,tech_name');
    ...
    $('dateFilter').addEventListener('change',renderReports);$('searchFilter').addEventListener('input',renderReports);$('clearFilters').addEventListener('click',()=>{$('dateFilter').value='';$('searchFilter').value='';renderReports()});$('refreshReports').addEventListener('click',
    async () => {
    await Promise.all([
    loadReports(),
    loadShiftReports()
    ...
    const
    tables=['report_games','report_defects','report_pc_status','report_inventory','report_spares','report_signoffs'];
    ...
    $('modalBody').innerHTML=`<div class="detail-card">
      <p class="error">${esc(err.message||'Database error')}</p>
    </div>`;
    }
    }
    ...
    d.defects.forEach(x => {
    const category = Number(x.pc_number) <= 10 ? 'VIP' : 'Standard' ; if (defectsBy[x.peripheral_type]?.[category]
      !==undefined) { defectsBy[x.peripheral_type][category]++; } }); const defectCards=[ 'Keyboard' , 'Headset'
      , 'Mouse' , 'Monitor' , 'Power Cord' ].map(type=> `
      ...
      const vip=d.pcs.filter(x=>x.pc_number>=1&&x.pc_number<=10&&x.no_defect).sort((a,b)=>a.pc_number-b.pc_number);
        const standard=d.pcs.filter(x=>x.pc_number>=11&&x.pc_number<=40&&x.no_defect).sort((a,b)=>
          a.pc_number-b.pc_number);
          const pcBlock=(title,list,total)=>`<div class="pc-status-block">
            <div class="pc-status-head"><strong>${title}</strong><span>${list.length}/${total} marked no defects</span>
            </div>
            <div class="pc-grid">${list.length?list.map(x=>`<span class="pc-chip">${pcLabel(x.pc_number)}
          ...
          d.spares.map(x => [x.item_type, x.quantity])
          );
          ...
          const spareRows = spareItems.map(item => `
          <div class="spare-row">
            <span>${item}</span>
            <strong>
              ${
              spareMap.has(item) &&
              spareMap.get(item) !== null &&
              spareMap.get(item) !== ''
              ? esc(spareMap.get(item))
              : '—'
              }
            </strong>
          </div>
          ...
          return `<div class="report-sheet" id="printReport">
            <div class="report-header">
              <div class="report-brand">
                <div class="report-logo">M</div>
                <div>
                  <div class="report-kicker">MARV’S GAMING HUB</div>
                  <h2>GAMING HUB OVERALL REPORT</h2>
                </div>
              </div>
              <div class="report-date"><small>REPORT
                  DATE</small><strong>${esc(prettyDateTime(r.created_at).split(',')[0])}</strong><span>Saved
                  ${esc(prettyDateTime(r.created_at))}</span></div>
            </div>
            ...
            <section class="report-section">
              <div class="report-section-title"><span>02</span>
                <h3>DEFECTIVE PERIPHERALS</h3>
              </div>
            ...
            <section class="report-section">
              <div class="report-section-title"><span>03</span>
                <h3>PC STATUS — NO DEFECTS</h3>
              </div>
              ${pcBlock('VIP PC 01–10',vip,10)}${pcBlock('STANDARD PC 11–40',standard,30)}
            </section>

            <section class="report-section">
              <div class="report-section-title"><span>04</span>
                <h3>OVERALL PERIPHERAL COUNT</h3>
              </div>
            ...
            <section class="report-section signoff-section">
              ...
              <div class="signoff-grid">
                <div class="signoff-card">
                  <div class="signoff-label">ADMIN</div>
                  <div class="signature-box">${d.sign.admin_signature?`<img class="signature"
                      src="${esc(d.sign.admin_signature)}" alt="Admin signature">`:'<span>No signature</span>'}</div>
                  <strong>${esc(d.sign.admin_name||'—')}</strong>
                </div>
                <div class="signoff-card">
                  <div class="signoff-label">TECH</div>
                  <div class="signature-box">${d.sign.tech_signature?`<img class="signature"
                      src="${esc(d.sign.tech_signature)}" alt="Tech signature">`:'<span>No signature</span>'}</div>
                  <strong>${esc(d.sign.tech_name||'—')}</strong>
                </div>
              </div>
            </section>

            <div class="report-footer"><span>MARV’S GAMING HUB • ADMIN ARCHIVE</span><span>Report ID:
                ${esc(r.id)}</span></div>
          </div>`;
          }

          function
          closeModal(){$('reportModal').classList.add('hidden');$('reportModal').setAttribute('aria-hidden','true')}
          ...
          <tr>
            ...
            <td class="report-actions">
              ...
              <button class="mini-view" data-shift-view="${esc(report.id)}">
                View
              </button>

              <button class="mini-delete" data-shift-delete="${esc(report.id)}">
                Delete
              </button>
          ...
          ? `
          <img class="signature" src="${esc(report.admin_signature)}" alt="Admin signature">
          `
          ...
          ? `
          <img class="signature" src="${esc(report.tech_signature)}" alt="Technician signature">
          `
          : '<span>No signature</span>';
          ...
          <div class="report-sheet" id="printReport">
            ...
            <section class="report-section">
              ...
              <div class="spare-grid">
                ...
                <div class="spare-row">
                  <span>Keyboard</span>
                  <strong>${esc(report.spare_keyboard ?? 0)}</strong>
                </div>

                <div class="spare-row">
                  <span>Mouse</span>
                  <strong>${esc(report.spare_mouse ?? 0)}</strong>
                </div>

                <div class="spare-row">
                  <span>Headset</span>
                  <strong>${esc(report.spare_headset ?? 0)}</strong>
                </div>

                <div class="spare-row">
                  <span>Power Cord</span>
                  <strong>${esc(report.spare_power_cord ?? 0)}</strong>
                </div>
